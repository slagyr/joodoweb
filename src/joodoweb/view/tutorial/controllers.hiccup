...
[:p "To make our posts controller and posts controller tester, create these two files: " [:b "src/sample_app/clj/controller/post_controller.clj"] " and " [:b "spec/clj/sample_app/controller/post_controller_spec.clj"]]
...
[:p "Remember - at this point your tests should fail"]
...
[:p "Let's now make a file called " [:b "20111215_test-post.hiccup"] " in the " [:b "/spec/clj/sample_app/view/test_posts"] " directory. After we do that, we are ready to write the controller logic to make those tests pass. Update the code in your post-controller so that it now looks like this:"]
[:pre {:class "brush: clojure"}
"(ns sample-app.controller.post-controller
